...
> funciona igual exceto onde indicado. Alternativa sem planilha: app `bacen/` já faz o núcleo.
...
Geração da fila (recalcular toda segunda, ou automático no app `bacen/`):
...
(coluna E) — o app `bacen/` já traz os dois primeiros prontos.
...
- Limitação honesta: gráficos nativos do Notion são fracos — mantenha o app `bacen/` ou a
  planilha para as curvas, e o Notion para captura e Kanban, ou use tudo no app `bacen/`.
